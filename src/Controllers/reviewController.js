...
const createReview = async function (req, res) {
    try {
        ...
        if (Object.keys(bodyData).length > 3) {
            return res.status(400).send({ status: false, message: "Please give only 3 inputs to create review" })
        }
...
const updateReview = async function (req, res) {
    try {
        ...
        if (!reviewExist) {
            return res.status(404).send({ status: false, message: "no such review exist" })
        }

        if (reviewExist.isDeleted == true) {
            return res.status(400).send({ status: false, message: "the review is already deleted" })
        }
...
const deleteReview = async function (req, res) {
    try {
        ...
        let deleteReview = await reviewModel.findOneAndUpdate({ _id: reviewId, bookId: bookId },
